...
export class GvConfigurator implements OnInit {
  ...
  primaryColors = computed<SurfacesType[]>(() => {
    const presetPalette = presets[this.store.preset() as KeyOfType<typeof presets>].primitive;
    const colors = [
      'emerald',
      'green',
      'lime',
  ...
  onMenuModeChange(event: string) {
    this.store.setMenuMode(event as any);
  }
}
