    ...
    template: `
        ...
        @if ($filled() && !$disabled() && showClear && !loading) {
          @if (!clearIconTemplate && !_clearIconTemplate) {
            <svg data-p-icon="times" [hBind]="ptm('clearIcon')" [class]="cx('clearIcon')" (click)="clear()" [attr.aria-hidden]="true" />
          }
          @if (clearIconTemplate || _clearIconTemplate) {
            <span [hBind]="ptm('clearIcon')" [class]="cx('clearIcon')" (click)="clear()" [attr.aria-hidden]="true">
                ...
                <h-chip
                  ...
                  <ng-template #removeicon>
                    @if (!removeIconTemplate && !_removeIconTemplate) {
                      <span [hBind]="ptm('chipIcon')" [class]="cx('chipIcon')" (click)="!readonly && !$disabled() ? removeOption($event, i) : ''">
                        <svg data-p-icon="times-circle" [class]="cx('chipIcon')" [attr.aria-hidden]="true" />
                      </span>
                    }
                    @if (removeIconTemplate || _removeIconTemplate) {
                      <span [hBind]="ptm('chipIcon')" [attr.aria-hidden]="true">
        ...
        @if (loading) {
          @if (!loadingIconTemplate && !_loadingIconTemplate) {
            <svg data-p-icon="spinner" [hBind]="ptm('loader')" [class]="cx('loader')" [spin]="true" [attr.aria-hidden]="true" />
          }
          @if (loadingIconTemplate || _loadingIconTemplate) {
            <span [hBind]="ptm('loader')" [class]="cx('loader')" [attr.aria-hidden]="true">
              <ng-template *ngTemplateOutlet="loadingIconTemplate || _loadingIconTemplate"></ng-template>
            </span>
          }
        }
        @if (dropdown) {
          <button #ddBtn type="button" [hBind]="ptm('dropdown')" [attr.aria-label]="dropdownAriaLabel" [class]="cx('dropdown')" [disabled]="$disabled()" hRipple (click)="handleDropdownClick($event)" [attr.tabindex]="tabindex">
            @if (dropdownIcon) {
              <span [ngClass]="dropdownIcon" [attr.aria-hidden]="true"></span>
            }
            @if (!dropdownIcon) {
              @if (!dropdownIconTemplate && !_dropdownIconTemplate) {
                <svg data-p-icon="chevron-down" [hBind]="ptm('dropdown')" />
              }
