...
import { blockBodyScroll, ConnectedOverlayScrollHandler, unblockBodyScroll } from '@gravionlabs/helix/dom';
import { CalendarIcon, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, ChevronUpIcon, TimesIcon } from '@gravionlabs/helix/icons';
...
import {
    ...
    Month,
    NavigationState
} from '@gravionlabs/helix/types/datepicker';
    ...
    template: `
        <ng-template [ngIf]="!inline">
            ...
            <ng-container *ngIf="showClear && !$disabled() && inputfieldViewChild?.nativeElement?.value">
                <svg data-p-icon="times" *ngIf="!clearIconTemplate && !_clearIconTemplate" [class]="cx('clearIcon')" [hBind]="ptm('inputIcon')" (click)="clear()" />
                <span *ngIf="clearIconTemplate || _clearIconTemplate" [class]="cx('clearIcon')" [hBind]="ptm('inputIcon')" (click)="clear()">
            ...
            <button
                ...
                *ngIf="showIcon && iconDisplay === 'button'"
                ...
                [hBind]="ptm('dropdown')"
            >
                <span *ngIf="icon" [ngClass]="icon" [hBind]="ptm('dropdownIcon')"></span>
                <ng-container *ngIf="!icon">
                    <svg data-p-icon="calendar" *ngIf="!triggerIconTemplate && !_triggerIconTemplate" [hBind]="ptm('dropdownIcon')" />
                    <ng-template *ngTemplateOutlet="triggerIconTemplate || _triggerIconTemplate"></ng-template>
                </ng-container>
            </button>
            <ng-container *ngIf="iconDisplay === 'input' && showIcon">
                <span [class]="cx('inputIconContainer')" [hBind]="ptm('inputIconContainer')" [attr.data-p]="inputIconDataP">
                    <svg data-p-icon="calendar" (click)="onButtonClick($event)" *ngIf="!inputIconTemplate && !_inputIconTemplate" [class]="cx('inputIcon')" [hBind]="ptm('inputIcon')" />

                    <ng-container *ngTemplateOutlet="inputIconTemplate || _inputIconTemplate; context: { clickCallBack: onButtonClick.bind(this) }"></ng-container>
                </span>
            </ng-container>
        </ng-template>
        <h-motion [visible]="inline || overlayVisible" name="p-anchored-overlay" [appear]="!inline" [options]="computedMotionOptions()" (onBeforeEnter)="onOverlayBeforeEnter($event)" (onAfterLeave)="onOverlayAfterLeave($event)">
            <div
                #contentWrapper
                [attr.id]="panelId"
                [ngStyle]="panelStyle"
            ...
            >
                ...
                <ng-container *ngIf="!timeOnly">
                    <div [class]="cx('calendarContainer')" [hBind]="ptm('calendarContainer')">
                        <div [class]="cx('calendar')" *ngFor="let month of months; let i = index" [hBind]="ptm('calendar')">
                            <div [class]="cx('header')" [hBind]="ptm('header')">
                                ...
                                <div [class]="cx('title')" [hBind]="ptm('title')">
                                    <button
                                    ...
                                    <button
                                        *ngIf="currentView !== 'year'"
                                        ...
                                        hRipple
                                        [hBind]="ptm('selectYear')"
                                        [attr.data-pc-group-section]="'navigator'"
                                    >
                                        {{ getYear(month) }}
                                    </button>
                                    <span [class]="cx('decade')" *ngIf="currentView === 'year'" [hBind]="ptm('decade')">
                            ...
                            <table [class]="cx('dayView')" role="grid" *ngIf="currentView === 'date'" [hBind]="ptm('table')">
                                <thead [hBind]="ptm('tableHeader')">
                                    <tr [hBind]="ptm('tableHeaderRow')">
                                        <th *ngIf="showWeek" [class]="cx('weekHeader')" [hBind]="ptm('weekHeader')">
                                            <span [hBind]="ptm('weekHeaderLabel')">{{ getTranslation('weekHeader') }}</span>
                                        </th>
                                        <th [class]="cx('weekDayCell')" scope="col" *ngFor="let weekDay of weekDays; let begin = first; let end = last" [hBind]="ptm('weekDayCell')">
                                            <span [class]="cx('weekDay')" [hBind]="ptm('weekDay')">{{ weekDay }}</span>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody [hBind]="ptm('tableBody')">
                                    <tr *ngFor="let week of month.dates; let j = index" [hBind]="ptm('tableBodyRow')">
                                        <td *ngIf="showWeek" [class]="cx('weekNumber')" [hBind]="ptm('weekNumber')">
                                            <span [class]="cx('weekLabelContainer')" [hBind]="ptm('weekLabelContainer')">
                                                {{ month.weekNumbers[j] }}
                                            </span>
                                        </td>
                                        <td *ngFor="let date of week" [attr.aria-label]="date.day" [class]="cx('dayCell', { date })" [hBind]="ptm('dayCell')">
                    ...
                    <div [class]="cx('monthView')" *ngIf="currentView === 'month'" [hBind]="ptm('monthView')">
                        <span *ngFor="let m of monthPickerValues(); let i = index" (click)="onMonthSelect($event, i)" (keydown)="onMonthCellKeydown($event, i)" [class]="cx('month', { month: m, index: i })" hRipple [hBind]="ptm('month')">
                            {{ m }}
                    ...
                    <div [class]="cx('yearView')" *ngIf="currentView === 'year'" [hBind]="ptm('yearView')">
                        <span *ngFor="let y of yearPickerValues()" (click)="onYearSelect($event, y)" (keydown)="onYearCellKeydown($event, y)" [class]="cx('year', { year: y })" hRipple [hBind]="ptm('year')">
                            {{ y }}
                ...
                <div [class]="cx('timePicker')" *ngIf="(showTime || timeOnly) && currentView === 'date'" [hBind]="ptm('timePicker')">
                    <div [class]="cx('hourPicker')" [hBind]="ptm('hourPicker')">
                        <h-button
                            rounded
                            variant="text"
                            severity="secondary"
                        ...
                        >
                            <ng-template #icon>
                                <svg data-p-icon="chevron-up" *ngIf="!incrementIconTemplate && !_incrementIconTemplate" [hBind]="ptm('pcIncrementButton')['icon']" />
                                <ng-template *ngTemplateOutlet="incrementIconTemplate || _incrementIconTemplate"></ng-template>
                            </ng-template>
                        </h-button>
                        <span [hBind]="ptm('hour')"><ng-container *ngIf="currentHour < 10">0</ng-container>{{ currentHour }}</span>
                        <h-button
                            rounded
                            variant="text"
                            severity="secondary"
                        ...
                        >
                            <ng-template #icon>
                                <svg data-p-icon="chevron-down" *ngIf="!decrementIconTemplate && !_decrementIconTemplate" [hBind]="ptm('pcDecrementButton')['icon']" />
                                <ng-template *ngTemplateOutlet="decrementIconTemplate || _decrementIconTemplate"></ng-template>
                            </ng-template>
                        </h-button>
                    </div>
                    <div class="p-datepicker-separator" [hBind]="ptm('separatorContainer')">
                        <span [hBind]="ptm('separator')">{{ timeSeparator }}</span>
                    </div>
                    <div [class]="cx('minutePicker')" [hBind]="ptm('minutePicker')">
                        <h-button
                            rounded
                            variant="text"
                            severity="secondary"
                        ...
                        <span [hBind]="ptm('minute')"><ng-container *ngIf="currentMinute < 10">0</ng-container>{{ currentMinute }}</span>
                        <h-button
                            rounded
                            variant="text"
                            severity="secondary"
                    ...
                    <div [class]="cx('separator')" *ngIf="showSeconds" [hBind]="ptm('separatorContainer')">
                        <span [hBind]="ptm('separator')">{{ timeSeparator }}</span>
                    </div>
                    <div [class]="cx('secondPicker')" *ngIf="showSeconds" [hBind]="ptm('secondPicker')">
                        <h-button
                            rounded
                            variant="text"
                            severity="secondary"
                        ...
                        <span [hBind]="ptm('second')"><ng-container *ngIf="currentSecond < 10">0</ng-container>{{ currentSecond }}</span>
                        <h-button
                            rounded
                            variant="text"
                            severity="secondary"
                    ...
                    <div [class]="cx('separator')" *ngIf="hourFormat == '12'" [hBind]="ptm('separatorContainer')">
                        <span [hBind]="ptm('separator')">{{ timeSeparator }}</span>
                    </div>
                    <div [class]="cx('ampmPicker')" *ngIf="hourFormat == '12'" [hBind]="ptm('ampmPicker')">
                        <h-button
                            text
                            rounded
                            severity="secondary"
                        ...
                        <span [hBind]="ptm('ampm')">{{ pm ? 'PM' : 'AM' }}</span>
                        <h-button
                            text
                            rounded
                            severity="secondary"
                ...
                <div [class]="cx('buttonbar')" *ngIf="showButtonBar" [hBind]="ptm('buttonbar')">
