        ...
        @if (!inline) {
          ...
          @if (showClear && !$disabled() && inputfieldViewChild?.nativeElement?.value) {
            @if (!clearIconTemplate && !_clearIconTemplate) {
              <svg data-p-icon="times" [class]="cx('clearIcon')" [hBind]="ptm('inputIcon')" (click)="clear()" />
            }
            @if (clearIconTemplate || _clearIconTemplate) {
              <span [class]="cx('clearIcon')" [hBind]="ptm('inputIcon')" (click)="clear()">
          ...
          @if (showIcon && iconDisplay === 'button') {
            <button
              ...
              >
              @if (icon) {
                <span [ngClass]="icon" [hBind]="ptm('dropdownIcon')"></span>
              }
              @if (!icon) {
                @if (!triggerIconTemplate && !_triggerIconTemplate) {
                  <svg data-p-icon="calendar" [hBind]="ptm('dropdownIcon')" />
                }
          ...
          @if (iconDisplay === 'input' && showIcon) {
            <span [class]="cx('inputIconContainer')" [hBind]="ptm('inputIconContainer')" [attr.data-p]="inputIconDataP">
              @if (!inputIconTemplate && !_inputIconTemplate) {
                <svg data-p-icon="calendar" (click)="onButtonClick($event)" [class]="cx('inputIcon')" [hBind]="ptm('inputIcon')" />
              }
        ...
        <h-motion [visible]="inline || overlayVisible" name="p-anchored-overlay" [appear]="!inline" [options]="computedMotionOptions()" (onBeforeEnter)="onOverlayBeforeEnter($event)" (onAfterLeave)="onOverlayAfterLeave($event)">
          <div
            ...
            @if (!timeOnly) {
              <div [class]="cx('calendarContainer')" [hBind]="ptm('calendarContainer')">
                @for (month of months; track month; let i = $index) {
                  <div [class]="cx('calendar')" [hBind]="ptm('calendar')">
                    <div [class]="cx('header')" [hBind]="ptm('header')">
                      ...
                      <div [class]="cx('title')" [hBind]="ptm('title')">
                        ...
                        @if (currentView === 'year') {
                          <span [class]="cx('decade')" [hBind]="ptm('decade')">
                    ...
                    @if (currentView === 'date') {
                      <table [class]="cx('dayView')" role="grid" [hBind]="ptm('table')">
                        <thead [hBind]="ptm('tableHeader')">
                          <tr [hBind]="ptm('tableHeaderRow')">
                            @if (showWeek) {
                              <th [class]="cx('weekHeader')" [hBind]="ptm('weekHeader')">
                            ...
                            @for (weekDay of weekDays; track weekDay; let begin = $first; let end = $last) {
                              <th [class]="cx('weekDayCell')" scope="col" [hBind]="ptm('weekDayCell')">
                        ...
                        <tbody [hBind]="ptm('tableBody')">
                          @for (week of month.dates; track week; let j = $index) {
                            <tr [hBind]="ptm('tableBodyRow')">
                              @if (showWeek) {
                                <td [class]="cx('weekNumber')" [hBind]="ptm('weekNumber')">
                              ...
                              @for (date of week; track date) {
                                <td [attr.aria-label]="date.day" [class]="cx('dayCell', { date })" [hBind]="ptm('dayCell')">
              ...
              @if (currentView === 'month') {
                <div [class]="cx('monthView')" [hBind]="ptm('monthView')">
                  @for (m of monthPickerValues(); track m; let i = $index) {
                    <span (click)="onMonthSelect($event, i)" (keydown)="onMonthCellKeydown($event, i)" [class]="cx('month', { month: m, index: i })" hRipple [hBind]="ptm('month')">
                      {{ m }}
              ...
              @if (currentView === 'year') {
                <div [class]="cx('yearView')" [hBind]="ptm('yearView')">
                  @for (y of yearPickerValues(); track y) {
                    <span (click)="onYearSelect($event, y)" (keydown)="onYearCellKeydown($event, y)" [class]="cx('year', { year: y })" hRipple [hBind]="ptm('year')">
                      {{ y }}
                      @if (isYearSelected(y)) {
                        <div class="p-hidden-accessible" aria-live="polite">
                          {{ y }}
            ...
            @if ((showTime || timeOnly) && currentView === 'date') {
              <div [class]="cx('timePicker')" [hBind]="ptm('timePicker')">
                  ...
                  <h-button
                    ...
                    <ng-template #icon>
                      @if (!incrementIconTemplate && !_incrementIconTemplate) {
                        <svg data-p-icon="chevron-up" [hBind]="ptm('pcIncrementButton')['icon']" />
                      }
                      <ng-template *ngTemplateOutlet="incrementIconTemplate || _incrementIconTemplate"></ng-template>
                    </ng-template>
                  </h-button>
                  <span [hBind]="ptm('hour')">@if (currentHour < 10) {
                    0
                  }{{ currentHour }}</span>
                  <h-button
                    rounded
                    ...
                    <ng-template #icon>
                      @if (!decrementIconTemplate && !_decrementIconTemplate) {
                        <svg data-p-icon="chevron-down" [hBind]="ptm('pcDecrementButton')['icon']" />
                      }
                ...
                <div [class]="cx('minutePicker')" [hBind]="ptm('minutePicker')">
                  ...
                  <span [hBind]="ptm('minute')">@if (currentMinute < 10) {
                    0
                  }{{ currentMinute }}</span>
                  <h-button
                    rounded
                ...
                @if (showSeconds) {
                  <div [class]="cx('separator')" [hBind]="ptm('separatorContainer')">
                    <span [hBind]="ptm('separator')">{{ timeSeparator }}</span>
                  </div>
                }
                @if (showSeconds) {
                  <div [class]="cx('secondPicker')" [hBind]="ptm('secondPicker')">
                    <h-button
                      rounded
                      variant="text"
                      severity="secondary"
                    ...
                    <span [hBind]="ptm('second')">@if (currentSecond < 10) {
                      0
                    }{{ currentSecond }}</span>
                    <h-button
                      rounded
                ...
                @if (hourFormat == '12') {
                  <div [class]="cx('ampmPicker')" [hBind]="ptm('ampmPicker')">
                    <h-button
                      text
                      rounded
                      severity="secondary"
            ...
            @if (showButtonBar) {
              <div [class]="cx('buttonbar')" [hBind]="ptm('buttonbar')">
