...
import { MenuItemTemplateContext, MenuPassThrough, MenuSubmenuHeaderTemplateContext } from '@gravionlabs/helix-core/types/menu';
...
@Component({
    ...
    templateUrl: './menuitemcontent.html',
...
export class Menu extends BaseComponent<MenuPassThrough> {
    ...
    onEndKey(event) {
        this.changeFocusedOptionIndex(find(this.containerViewChild()?.nativeElement, 'li[data-pc-section="item"][data-h-disabled="false"]').length - 1);
        event.preventDefault();
    }
    ...
    findNextOptionIndex(index) {
        const links = find(this.containerViewChild()?.nativeElement, 'li[data-pc-section="item"][data-h-disabled="false"]');
