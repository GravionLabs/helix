...
import { addClass, getHeight, getOuterHeight, getOuterWidth, getWidth, hasClass, isRTL, removeClass } from '@gravionlabs/helix-core/uix/utils';
...
import type { SplitterResizeEndEvent, SplitterResizeStartEvent } from '@gravionlabs/helix-core/types/splitter';
import { SplitterStyle } from './style/splitterstyle';
import { SplitterPassThrough } from '@gravionlabs/helix-core/types/splitter';
...
export class Splitter extends BaseComponent<SplitterPassThrough> {
     ...
     * @group Props
     */
    readonly stateStorage = input<string | undefined>('session');
    /**
     * Storage identifier of a stateful Splitter.
     * @group Props
     */
    readonly stateKey = input<string | undefined | null>(null);
    /**
     * Orientation of the panels. Valid values are 'horizontal' and 'vertical'.
     * @group Props
     */
    readonly layout = input<string | undefined>('horizontal');
    /**
    ...
    validateResize(newPrevPanelSize: number, newNextPanelSize: number) {
        ...
        if (this.minSizes().length >= 1 && minSizes[0] && minSizes[0] > newPrevPanelSize) {
            return false;
        }

        const minSizesValue = this.minSizes();
        if (this.minSizes().length > 1 && minSizesValue[1] && minSizesValue[1] > newNextPanelSize) {
            return false;
        }
