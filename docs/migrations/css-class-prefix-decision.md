...
> **Superseded (epic #421, feature #431, 2026-09-14):** the blocking
> dependency described below — the un-forked `@primeuix/styles`/`@primeuix/styled`
> layer hardcoding `.p-*` — no longer holds. Epic #421 vendored `styles`,
> `styled`, `utils`, `motion` and `themes` into `helix-core`, and feature
> #431 executed the rename: `.p-*` → `.h-*`, `--p-*` → `--h-*`,
> `data-p-*`/`data-p` → `data-h-*`/`data-h`, across `projects/core`,
> `projects/shell`, `projects/zod`, `projects/ag-grid` and
> `apps/helix-demo`. The `tailwindcss-primeui` Tailwind plugin (used by
> `projects/shell`) was vendored and renamed too — see
> `projects/shell/VENDOR.md`. This document's cost/benefit analysis below
> is preserved as the historical record of why the rename was infeasible
> before #421.
