...
| --- | --- | --- |
| `onNodeExpand` | `output&lt;TreeSelectNodeExpandEvent&gt;()` | Callback to invoke when a node is expanded. |
| `onNodeCollapse` | `output&lt;TreeSelectNodeCollapseEvent&gt;()` | Callback to invoke when a node is collapsed. |
| `onShow` | `output&lt;any&gt;()` | Callback to invoke when the overlay is shown. |
| `onHide` | `output&lt;Event&gt;()` | Callback to invoke when the overlay is hidden. |
| `onClear` | `output&lt;any&gt;()` | Callback to invoke when input field is cleared. |
| `onFilter` | `output&lt;TreeFilterEvent&gt;()` | Callback to invoke when data is filtered. |
| `onFocus` | `output&lt;Event&gt;()` | Callback to invoke when treeselect gets focus. |
| `onBlur` | `output&lt;Event&gt;()` | Callback to invoke when treeselect loses focus. |
