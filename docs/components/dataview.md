...
| `rowsPerPageOptions` | `any[] \| number[] \| undefined` | — | Array of integer/object values to display inside rows per page dropdown of paginator |
...
| --- | --- | --- |
| `onLazyLoad` | `output&lt;DataViewLazyLoadEvent&gt;()` | Callback to invoke when paging, sorting or filtering happens in lazy mode. |
| `onPage` | `output&lt;DataViewPageEvent&gt;()` | Callback to invoke when pagination occurs. |
| `onSort` | `output&lt;DataViewSortEvent&gt;()` | Callback to invoke when sorting occurs. |
| `onChangeLayout` | `output&lt;DataViewLayoutChangeEvent&gt;()` | Callback to invoke when changing layout. |
