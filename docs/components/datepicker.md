...
| `dateFormat` | `string \| undefined` | — | Format of the date which can also be defined at locale settings. |
...
| `minDate` | `Date \| null \| undefined` | — | The minimum selectable date. |
| `maxDate` | `Date \| null \| undefined` | — | The maximum selectable date. |
| `disabledDates` | `Date[] \| undefined` | — | Array with dates that should be disabled (not selectable). |
| `disabledDays` | `number[] \| undefined` | — | Array with weekday numbers that should be disabled (not selectable). |
| `showTime` | `boolean \| undefined` | — | Whether to display timepicker. |
| `responsiveOptions` | `DatePickerResponsiveOptions[] \| undefined` | — | An array of options for responsive design. |
| `numberOfMonths` | `number` | `1` | Number of months to display. |
| `firstDayOfWeek` | `number \| undefined` | — | Defines the first of the week for various date calculations. |
| `view` | `DatePickerTypeView` | `'date'` | Type of view to display, valid values are "date" for datepicker and "month" for month picker. |
| `defaultDate` | `Date \| null \| undefined` | — | Set the date to highlight on first opening if the field is blank. |
...
| --- | --- | --- |
| `onFocus` | `output&lt;Event&gt;()` | Callback to invoke on focus of input field. |
| `onBlur` | `output&lt;Event&gt;()` | Callback to invoke on blur of input field. |
| `onClose` | `output&lt;HTMLElement&gt;()` | Callback to invoke when date panel closed. |
| `onSelect` | `output&lt;Date&gt;()` | Callback to invoke on date select. |
| `onClear` | `output&lt;void&gt;()` | Callback to invoke when input field cleared. |
| `onInput` | `output&lt;any&gt;()` | Callback to invoke when input field is being typed. |
| `onTodayClick` | `output&lt;Date&gt;()` | Callback to invoke when today button is clicked. |
| `onClearClick` | `output&lt;any&gt;()` | Callback to invoke when clear button is clicked. |
| `onMonthChange` | `output&lt;DatePickerMonthChangeEvent&gt;()` | Callback to invoke when a month is changed using the navigators. |
| `onYearChange` | `output&lt;DatePickerYearChangeEvent&gt;()` | Callback to invoke when a year is changed using the navigators. |
| `onClickOutside` | `output&lt;any&gt;()` | Callback to invoke when clicked outside of the date panel. |
| `onShow` | `output&lt;HTMLElement&gt;()` | Callback to invoke when datepicker panel is shown. |
