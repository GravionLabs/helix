...
```ts
import { ConfirmDialog } from '@gravionlabs/helix-core/confirmdialog';
```
...
| `visible` | `any` | `false` | Current visible state as a boolean. |
| `position` | `"left" \| "right" \| "top" \| "bottom" \| "center" \| "topleft" \| "topright" \| "bottomleft" \| "bottomright"` | `'center'` | Allows getting the position of the component. |
...
| --- | --- | --- |
| `onHide` | `output&lt;ConfirmEventType \| undefined&gt;()` | Callback to invoke when dialog is hidden. |
