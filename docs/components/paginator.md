...
```ts
import { Paginator } from '@gravionlabs/helix-core/paginator';
```
...
| `dropdownAppendTo` | `any` | — | Target element to attach the dropdown overlay, valid values are "body" or a local ng-template variable of another element (note: use binding with brackets for template variables, e.g. [appendTo]="mydiv" for a div element having #mydiv as variable name). |
...
| --- | --- | --- |
| `onPageChange` | `output&lt;PaginatorState&gt;()` | Callback to invoke when page changes, the event object contains information about the new state. |
