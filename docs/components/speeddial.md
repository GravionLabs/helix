...
| `direction` | `"left" \| "right" \| "up" \| "down" \| "up-left" \| "up-right" \| "down-left" \| "down-right" \| undefined` | `'up'` | Specifies the opening direction of actions. |
| `transitionDelay` | `number` | `30` | Transition delay step for each action item. |
| `type` | `"circle" \| "linear" \| "semi-circle" \| "quarter-circle" \| undefined` | `'linear'` | Specifies the opening type of actions. |
...
| `tooltipOptions` | `TooltipOptions` | `undefined!` | Whether to display the tooltip on items. The modifiers of Tooltip can be used like an object in it. Valid keys are 'event' and 'position'. |
...
| --- | --- | --- |
| `onVisibleChange` | `output&lt;boolean&gt;()` | Fired when the visibility of element changed. |
| `onClick` | `output&lt;MouseEvent&gt;()` | Fired when the button element clicked. |
| `onShow` | `output&lt;Event \| undefined&gt;()` | Fired when the actions are visible. |
| `onHide` | `output&lt;Event \| undefined&gt;()` | Fired when the actions are hidden. |
